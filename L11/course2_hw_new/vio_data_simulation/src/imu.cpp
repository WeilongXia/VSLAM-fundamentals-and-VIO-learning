...
void IMU::testImu(std::string src, std::string dist)
{
    ...
    for (int i = 1; i < imudata.size(); ++i)
    {
        ...
        // Eigen::Vector3d acc_w = Qwb * (imu_data1.imu_acc) + gw; // aw = Rwb * ( acc_body - acc_bias ) + gw
        // Qwb = Qwb * dq;
        ...
        Eigen::Quaterniond dq;
        Eigen::Vector3d dtheta_half = (imu_data0.imu_gyro + imu_data1.imu_gyro) * dt / 4.0;
        dq.w() = 1;
        dq.x() = dtheta_half.x();
        dq.y() = dtheta_half.y();
        dq.z() = dtheta_half.z();
        ...
        Eigen::Vector3d acc_w = (Qwb * dq * (imu_data1.imu_acc) + gw + Qwb * (imu_data0.imu_acc) + gw) / 2;
        Qwb = Qwb * dq;
        Pwb = Pwb + Vw * dt + 0.5 * dt * dt * acc_w;
        Vw = Vw + dt * acc_w;
        ...
        save_points << imu_data1.timestamp << " " << Qwb.w() << " " << Qwb.x() << " " << Qwb.y() << " " << Qwb.z()

...
// global variables
string first_file = "../pic/1.png";
string second_file = "../pic/2.png";
...
// 打印函数执行时间
template <typename FuncT> void evaluate_and_call(FuncT func, const std::string &func_name = "", int times = 10)
{
    double total_time = 0;
    ...
    std::cout << "方法 " << func_name << " 平均调用时间/次数: " << total_time / times << "/" << times << " 毫秒."
              << std::endl;
}
...
int main(int argc, char **argv)
{
    ...
    evaluate_and_call([&]() { computeAngleMT(first_image, keypoints); }, "compute angle mt", 1);
    ...
    evaluate_and_call([&]() { computeORBDescMT(first_image, keypoints, descriptors_mt); }, "compute orb descriptor mt",
                      1);
    ...
    evaluate_and_call([&]() { bfMatch(descriptors, descriptors2, matches); }, "bf match", 1);
...
void computeAngle(const cv::Mat &image, vector<cv::KeyPoint> &keypoints)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        if (kp.pt.x < half_patch_size || kp.pt.y < half_patch_size || kp.pt.x > image.cols - half_patch_size ||
        ...
        else
        {
            ...
            for (int u = -half_patch_size; u < half_patch_size; ++u)
            {
                m_10 += u * center[u - step * half_patch_size];
                m_01 += (-half_patch_size) * center[u - step * half_patch_size];
            }

            kp.angle = std::atan2(m_10, m_01) / pi * 180.0f;
            // std::cout << "kp.angle: " << kp.angle << std::endl;
        }
...
void computeORBDesc(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc)
{
    ...
    for (auto &kp : keypoints)
    {
        ...
        for (int i = 0; i < 256; i++)
        {
            ...
            int u_p = ORB_pattern[4 * i], v_p = ORB_pattern[4 * i + 1], u_q = ORB_pattern[4 * i + 2],
                v_q = ORB_pattern[4 * i + 3];

            int u_p_rotated = cvRound(cos(kp.angle * pi / 180) * u_p - sin(kp.angle * pi / 180) * v_p + kp.pt.x);
            int v_p_rotated = cvRound(sin(kp.angle * pi / 180) * u_p + cos(kp.angle * pi / 180) * v_p + kp.pt.y);
            int u_q_rotated = cvRound(cos(kp.angle * pi / 180) * u_q - sin(kp.angle * pi / 180) * v_q + kp.pt.x);
            int v_q_rotated = cvRound(sin(kp.angle * pi / 180) * u_q + cos(kp.angle * pi / 180) * v_q + kp.pt.y);

            if (u_p_rotated < 0 || u_p_rotated >= image.cols || u_q_rotated < 0 || u_q_rotated >= image.cols ||
                v_p_rotated < 0 || v_p_rotated >= image.rows || v_q_rotated < 0 || v_q_rotated >= image.rows)
            {
                IFOutBound = true;
                break;
            }
            ...
            {
                d[i] = (image.at<uchar>(v_p_rotated, u_p_rotated) <= image.at<uchar>(v_q_rotated, u_q_rotated));
            }
    ...
    std::cout << "bad/total: " << bad << "/" << desc.size() << std::endl;
    return;
}
...
void computeORBDescMT(const cv::Mat &image, vector<cv::KeyPoint> &keypoints, vector<DescType> &desc)
{
    ...
    std::for_each(std::execution::par_unseq, keypoints.begin(), keypoints.end(), [&desc, &image](auto &kp) {
        DescType d(256, false);
        bool IFOutBound = false;
...
void bfMatch(const vector<DescType> &desc1, const vector<DescType> &desc2, vector<cv::DMatch> &matches)
{
    ...
    for (int i1 = 0; i1 < desc1.size(); ++i1)
    {
        ...
        for (int i2 = 0; i2 < desc2.size(); ++i2)
        {
            ...
            for (int j = 0; j < 256; ++j)
            {
                hamming_dist += (desc1[i1][j] == desc2[i2][j]) ? 0 : 1;
            }
            // std::cout << "hamming_dist: " << hamming_dist << std::endl;

            if (hamming_dist < d_max && hamming_dist < m.distance)
            {
                m.distance = hamming_dist;
                m.trainIdx = i2;
            }
        }

...
template <typename T> inline void CrossProduct(const T x[3], const T y[3], T result[3])
{
...
// reference: https://www.euclideanspace.com/maths/geometry/rotations/conversions/angleToQuaternion/index.htm
template <typename T> inline void AngleAxisToQuaternion(const T *angle_axis, T *quaternion)
{
    ...
    if (theta_squared > T(std::numeric_limits<double>::epsilon()))
    {
        ...
        const T k = sin(half_theta);
        quaternion[0] = cos(half_theta);
        quaternion[1] = a0 / theta * k;
        quaternion[2] = a1 / theta * k;
        quaternion[3] = a2 / theta * k;
    }
    else
...
template <typename T> inline void QuaternionToAngleAxis(const T *quaternion, T *angle_axis)
{
...
template <typename T> inline void AngleAxisRotatePoint(const T angle_axis[3], const T pt[3], T result[3])
{
    ...
    {
        // The rotation matrix is the Exponential map of the correspond rotation vector(angle axis)
        // When theta is near zero, wo can neglect the second order term and so on.

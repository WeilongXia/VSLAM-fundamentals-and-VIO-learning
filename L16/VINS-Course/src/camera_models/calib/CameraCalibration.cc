...
void CameraCalibration::drawResults(std::vector<cv::Mat> &images) const
{
    ...
    for (size_t i = 0; i < images.size(); ++i)
    {
        ...
        for (size_t j = 0; j < m_imagePoints.at(i).size(); ++j)
        {
            ...
            cv::circle(image, cv::Point(cvRound(pObs.x * drawMultiplier), cvRound(pObs.y * drawMultiplier)), 5, green,
                       2, cv::LINE_AA, drawShiftBits);

            cv::circle(image, cv::Point(cvRound(pEst.x * drawMultiplier), cvRound(pEst.y * drawMultiplier)), 5, red, 2,
...
bool CameraCalibration::calibrateHelper(CameraPtr &camera, std::vector<cv::Mat> &rvecs,

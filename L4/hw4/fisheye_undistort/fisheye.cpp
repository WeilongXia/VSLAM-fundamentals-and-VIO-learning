...
// 文件路径，如果不对，请调整
std::string input_file = "../pic/fisheye.jpg";
...
int main(int argc, char **argv)
{
    ...
    for (int v = 0; v < rows; v++)
        for (int u = 0; u < cols; u++)
        {
            ...
            double theta = atan(sqrt(r_square));

            double theta_d =
                theta * (1 + k1 * pow(theta, 2) + k2 * pow(theta, 4) + k3 * pow(theta, 6) + k4 * pow(theta, 8));

            double x_distorted = (theta_d / sqrt(r_square)) * x, y_distorted = (theta_d / sqrt(r_square)) * y;
    ...
    std::cout << "image undistort cost " << (float)(t2 - t1) / CLOCKS_PER_SEC << std::endl;

    // 画图去畸变后图像
    cv::imshow("image undistorted", image_undistort);
    cv::imwrite("../pic/fisheye_undist.jpg", image_undistort);
    cv::waitKey();

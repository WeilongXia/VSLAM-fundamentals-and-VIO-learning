...
void GetSimDataInWordFrame(vector<Frame> &cameraPoses, vector<Eigen::Vector3d> &points)
{
    ...
    {
        std::uniform_real_distribution<double> xy_rand(-4, 4.0); // [-4.0, 4.0)
        std::uniform_real_distribution<double> z_rand(4., 8.);   // [4.0, 8.0)
...
int main()
{
    ...
    {
        std::cout << "after opt, point " << k << " : gt " << 1. / points[k].z() << " ,noise " << noise_invd[k]

...
void System::PubImageData(double dStampSec, std::string filename)
{
    ...
    TicToc t_r;
    // cout << "3 PubImageData t : " << dStampSec << endl;
    trackerData[0].readImage(filename, dStampSec);
    ...
    if (PUB_THIS_FRAME)
    {
        ...
        for (int i = 0; i < NUM_OF_CAM; i++)
        {
            ...
            for (unsigned int j = 0; j < un_pts.size(); j++)
            {
                // if (trackerData[i].track_cnt[j] > 1)
                {
                    // int p_id = ids[j];
                    ...
                    double z = 1;
                    feature_points->points.push_back(Vector3d(x, y, z));
                    feature_points->id_of_point.push_back(j * NUM_OF_CAM + i);
                    feature_points->u_of_point.push_back(cur_pts[j].x);
                    feature_points->v_of_point.push_back(cur_pts[j].y);
...
void System::Draw()
{
    ...
    pangolin::OpenGlRenderState s_cam(pangolin::ProjectionMatrix(1024, 768, 500, 500, 512, 384, 0.1, 1000),

...
template <typename Scalar> class CameraObserve
{
  public:
    // 对应的相机 Pose 节点
  ...
  public:
    /* 构造函数与析构函数 */
    CameraObserve(std::shared_ptr<VertexCameraPose<Scalar>> camera, const Vector2<Scalar> &norm,
                  const Matrix2<Scalar> &infoMatrix, std::shared_ptr<KernelBase<Scalar>> kernel = nullptr)
    {
        this->camera = camera;
        this->norm = norm;
